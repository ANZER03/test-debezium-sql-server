...
and collects per-container system metrics:
  ...
  - Memory MB    (usage in MB, limit in MB, usage %)
...
def _parse_stats(raw: dict, prev_raw: dict | None) -> dict:
    ...
    prev_raw is the previous snapshot (needed for delta calculations).
    """
    result = {}
    ...
    try:
        ...
        # subtract cache (Linux reports RSS + cache in usage)
...
class SystemCollector:
    ...
    def __init__(
        ...
        # {container_name: [{"ts": float, "elapsed": float, "cpu_pct": float, ...}, ...]}
        self._data: dict[str, list] = {c: [] for c in containers}
        self._prev: dict[str, dict] = {}  # previous raw snapshot per container
    ...
    def stop(self):
        ...
        print(
            f"  [SystemCollector] Stopped — {sum(len(v) for v in self._data.values())} total samples"
        )
        ...
        while not self._stop_event.is_set():
            ...
            for container in self.containers:
                try:
                    ...
                    # Container might not be up yet; skip silently
                    pass
