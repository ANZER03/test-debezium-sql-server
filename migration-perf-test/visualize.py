...
visualize.py
============
Reads migration_results_latest.json and produces a full observability dashboard.

Static PNG charts (saved to results/charts/):
  01  throughput_msg_rate.png     — msg/s over time, all 5 cases
  02  throughput_mb_rate.png      — MB/s over time, all 5 cases
  03  cumulative_messages.png     — progress curve, all 5 cases
  ...
  10  sys_connect_cpu.png         — kafka-connect CPU %
  11  sys_connect_mem.png         — kafka-connect RAM MB
  12  sys_connect_net_rx.png      — kafka-connect Network RX MB/s
  13  sys_connect_net_tx.png      — kafka-connect Network TX MB/s
  14  sys_connect_disk_r.png      — kafka-connect Disk Read MB/s
  15  sys_connect_disk_w.png      — kafka-connect Disk Write MB/s
  16  sys_kafka_cpu.png           — Kafka broker CPU %
  17  sys_kafka_mem.png           — Kafka broker RAM MB
  18  sys_kafka_net_tx.png        — Kafka broker Network TX MB/s
  19  sys_sql_cpu.png             — SQL Server CPU %
  20  sys_sql_mem.png             — SQL Server RAM MB
  21  sys_sql_disk_r.png          — SQL Server Disk Read MB/s
  22  connect_task_states.png     — Connect task states over time
  23  consumer_lag.png            — Consumer group lag over time
  24  heatmap_kafka.png           — Kafka metrics heatmap (all cases)
  25  heatmap_system.png          — System resource heatmap
  26  radar_chart.png             — Radar / spider chart (5 dimensions)
  27  sys_all_cpu_panel.png       — All containers × all cases CPU panel
  28  sys_all_net_tx_panel.png    — All containers × all cases Net TX panel

Interactive HTML:
  dashboard.html                  — Full Plotly dashboard (all panels, zoomable)
"""
...
# Palette — 5 cases
# ---------------------------------------------------------------------------
CASE_COLOURS = ["#4C72B0", "#DD8452", "#55A868", "#C44E52", "#9467BD"]
CASE_MARKERS = ["o", "s", "^", "D", "P"]
CONTAINER_COLOURS = {
...
def save_fig(fig, name: str, out_dir: pathlib.Path):
    p = out_dir / name
...
def sys_series(r: dict, container: str, key: str, w=3):
...
def avg_sys(r: dict, container: str, key: str) -> float:
    samples = r.get("system", {}).get(container, [])
...
# ---------------------------------------------------------------------------
# Section D — Connect-specific charts (task states, consumer lag)
# ---------------------------------------------------------------------------


def connect_task_states_chart(results, out_dir):
    """Stacked area showing tasks_running vs tasks_failed over time per case."""
    fig, axes = plt.subplots(
    ...
    for i, r in enumerate(results):
        ax = axes[i]
        samples = r.get("connect_metrics", [])
        if not samples:
            ax.set_title(short(r), fontsize=9)
            continue
        x = [s["elapsed"] for s in samples]
        running = [s.get("tasks_running", 0) for s in samples]
        failed = [s.get("tasks_failed", 0) for s in samples]
        ax.fill_between(x, running, alpha=0.6, color="#55A868", label="Running")
        ax.fill_between(x, failed, alpha=0.6, color="#C44E52", label="Failed")
        ax.set_title(short(r), fontsize=9, fontweight="bold", color=CASE_COLOURS[i])
        ...
        ax.grid(True, alpha=0.3, linestyle="--")
        ax.tick_params(labelsize=7)
    fig.suptitle("Connector Task States Over Time", fontsize=13, fontweight="bold")
    fig.tight_layout()
    save_fig(fig, "22_connect_task_states.png", out_dir)


def consumer_lag_chart(results, out_dir):
    """Consumer group lag over time per case (from ConnectMetricsCollector)."""
    fig, ax = plt.subplots(figsize=(12, 4.5))
    has_data = False
    for i, r in enumerate(results):
        samples = r.get("connect_metrics", [])
        valid = [
            (s["elapsed"], s["consumer_lag"])
            for s in samples
    ...
    if not has_data:
        plt.close(fig)
        return
    ax.set_title("Consumer Group Lag Over Time", fontsize=13, fontweight="bold", pad=8)
    ax.set_xlabel("Elapsed time (s)", fontsize=11)
    ax.set_ylabel("Unread messages (lag)", fontsize=11)
    ax.yaxis.set_major_formatter(mticker.FuncFormatter(lambda v, _: f"{int(v):,}"))
    ax.legend(fontsize=9)
    ax.grid(True, alpha=0.3, linestyle="--")
    fig.tight_layout()
    save_fig(fig, "23_consumer_lag.png", out_dir)
...
def heatmap_kafka(results, out_dir):
    ...
    fig, ax = plt.subplots(figsize=(11, max(3, len(results) * 0.9)))
    ...
    fig.tight_layout()
    save_fig(fig, "24_heatmap_kafka.png", out_dir)
...
def heatmap_system(results, out_dir):
    ...
    norm_mat = np.zeros_like(matrix)
    for j in range(matrix.shape[1]):
        col = matrix[:, j]
        mn, mx = col.min(), col.max()
        norm_mat[:, j] = (col - mn) / (mx - mn) if mx > mn else 0.5

    fig, ax = plt.subplots(
        figsize=(max(12, len(col_labels) * 1.1), max(4, len(results) * 0.9))
    )
    im = ax.imshow(norm_mat, cmap="YlOrRd", aspect="auto", vmin=0, vmax=1)
    ...
    for i in range(matrix.shape[0]):
        for j in range(matrix.shape[1]):
            ...
            fc = "white" if norm_mat[i, j] > 0.65 else "black"
    ...
    fig.tight_layout()
    save_fig(fig, "25_heatmap_system.png", out_dir)
...
def radar_chart(results, out_dir):
    ...
    def lat_score():
        ...
        for r in results:
            ...
            vals.append(statistics.mean(ts) if ts else 9999)
        mn, mx = min(vals), max(vals)
        if mx == mn:
            return [0.5] * len(results)
    ...
    fig.tight_layout()
    save_fig(fig, "26_radar_chart.png", out_dir)


# ---------------------------------------------------------------------------
# Section G — Interactive Plotly dashboard
# ---------------------------------------------------------------------------
...
def plotly_dashboard(results, out_dir):
    ...
    def add_connect_metric(metric_key, row, col):
        for i, r in enumerate(results):
            samples = r.get("connect_metrics", [])
            valid = [
                (s["elapsed"], s.get(metric_key, 0))
                for s in samples
                if s.get(metric_key, -1) >= 0
            ]
            if not valid:
                continue
            x, y = zip(*valid)
    ...
    # Row 8 — Connect-level
    add_connect_metric("consumer_lag", 8, 1)
    add_connect_metric("tasks_running", 8, 2)
    # Row 9
    add_sys("kafka-connect", "net_rx_mbps", 9, 1)
    add_sys("sqlserver", "disk_read_mbps", 9, 2)
    ...
    for (row, col), lbl in ylabels.items():
    ...
    fig.update_layout(
        title=dict(
            ...
            "<sup>5 Tuning Cases × 2M rows | Kafka + Connect + System metrics</sup>",
            x=0.5,
            font=dict(size=17),
        ),
        height=4500,
...
def main():
    path = (
        ...
        if len(sys.argv) > 1
        else pathlib.Path(__file__).parent / "results" / "migration_results_latest.json"
    )
    if not path.exists():
    ...
    results = load(path)
    if not results:
        print("No valid results in file.")
        sys.exit(1)
    ...
    print("\n  Connect metrics:")
    connect_task_states_chart(results, out_dir)
    consumer_lag_chart(results, out_dir)
